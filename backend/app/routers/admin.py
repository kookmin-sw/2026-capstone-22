...
async def update_user(
    ...
    if user_update.group_id is not None:
        if (
            ...
            and not db.query(Group)
            .filter(
                Group.id == user_update.group_id,
...
async def create_group(
    ...
    existing = (
        db.query(Group)
        .filter(Group.name == group.name, Group.tenant_id == current_user.tenant_id)
        .first()
    )
    if existing:
        raise HTTPException(
    ...
    new_group = Group(
        name=group.name, description=group.description, tenant_id=current_user.tenant_id
    )
    db.add(new_group)
    db.commit()
...
async def get_group(
    ...
    group = (
        db.query(Group)
        .filter(Group.id == group_id, Group.tenant_id == current_user.tenant_id)
        .first()
    )
    if not group:
        raise HTTPException(status_code=404, detail="Group not found")
...
@router.post(
    "/store-permissions",
    response_model=StorePermissionResponse,
...
@router.delete(
    "/store-permissions/{permission_id}", status_code=status.HTTP_204_NO_CONTENT
)

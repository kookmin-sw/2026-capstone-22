...
from ..schemas.corpus import (
    CorpusCreate, CorpusResponse, DocumentResponse, CorpusDetailResponse,
    DocumentUploadResponse, CorpusDeleteRequest, BulkDeleteRequest, BulkDeleteResponse,
    CorpusUpdate,
)
...
async def create_corpus(
    ...
    try:
        ...
        if search_backend == "vertex_ai_search":
            ...
            import re
            safe_name = re.sub(r'[^a-z0-9-]', '', corpus_data.display_name.lower().replace(" ", "-"))
            ...
            weaviate_endpoint = db.query(PlatformSetting).filter(PlatformSetting.key == "WEAVIATE_HTTP_ENDPOINT").first()
            weaviate_collection = db.query(PlatformSetting).filter(PlatformSetting.key == "WEAVIATE_COLLECTION_NAME").first()
            weaviate_secret = db.query(PlatformSetting).filter(PlatformSetting.key == "WEAVIATE_API_KEY_SECRET").first()

            weaviate_config = None
            if weaviate_endpoint and weaviate_endpoint.value and weaviate_collection and weaviate_collection.value and weaviate_secret and weaviate_secret.value:
                weaviate_config = {
                ...
                logger.info(f"Creating corpus with Weaviate backend: {weaviate_endpoint.value}")
        ...
        try:
            ...
            if tenant and gcs_service.is_configured(tenant_id=current_user.tenant_id, db=db):
                corpus_folder = corpus_data.display_name.replace("/", "_")
                placeholder_path = f"tenants/{tenant.slug}/{corpus_folder}/.keep"
                gcs_client, bucket_name = gcs_service._get_tenant_gcs(current_user.tenant_id, db)
                if bucket_name:
...
async def list_corpora(
    ...
    try:
        ...
        ).outerjoin(Document).group_by(Corpus.id).all()
...
async def update_corpus_settings(
    ...
    try:
        ...
        logger.info(f"Updated corpus settings: {corpus_name} (is_public={corpus.is_public})")
...
async def delete_corpus(
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"External resource deletion failed for {corpus_name} (proceeding with DB cleanup): {ext_err}")
        ...
        try:
            ...
            if tenant:
                ...
                if gcs_service.is_configured(tenant_id=current_user.tenant_id, db=db):
                    corpus_obj = db.query(Corpus).filter(
                        Corpus.corpus_name == corpus_name,
                        Corpus.tenant_id == current_user.tenant_id
                    ).first()
                    if corpus_obj:
                        ...
                        if bucket_name:
                            ...
                            for blob in blobs:
                                try:
                                    ...
                                    pass
                            if blobs:
                                logger.info(f"Deleted {len(blobs)} GCS objects under {prefix}")
        except Exception as gcs_err:
            logger.warning(f"GCS folder cleanup failed for corpus {corpus_name}: {gcs_err}")
        ...
        if corpus:
            ...
            db.query(Document).filter(Document.corpus_id == corpus.id).delete(synchronize_session=False)
            db.delete(corpus)
            db.commit()
...
async def upload_document(
    ...
    if custom_metadata:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid custom_metadata JSON: {str(e)}")
    ...
    try:
        ...
        logger.info(f"Upload MIME type: {raw_mime} -> {mime_type} (GCS: {gcs_content_type})")
        ...
        if not gcs_service.is_configured(tenant_id=current_user.tenant_id, db=db):
            raise HTTPException(status_code=400, detail="GCS 버킷이 설정되지 않았습니다. 문서 업로드에는 GCS 설정이 필요합니다.")
        ...
        try:
            ...
            db_corpus = db.query(CorpusModel).filter(CorpusModel.corpus_name == corpus_name).first()
            raw_folder = db_corpus.display_name if db_corpus else corpus_name.split("/")[-1]
            corpus_folder = raw_folder.replace("/", "_")
            gcs_object_path = f"tenants/{tenant_slug}/{corpus_folder}/{safe_filename}"
            gcs_path = gcs_service.upload_file(file_path, gcs_object_path, content_type=gcs_content_type, tenant_id=current_user.tenant_id, db=db)
            bucket_name = gcs_service.get_bucket_name(tenant_id=current_user.tenant_id, db=db)
            ...
            raise HTTPException(status_code=500, detail=f"GCS 업로드 실패: {str(gcs_err)}")
        ...
        if search_backend == "vertex_ai_search":
            ...
            corpus_record = db.query(CorpusModel2).filter(
                CorpusModel2.corpus_name == corpus_name
            ).first()
            if corpus_record:
                doc_name = f"{corpus_name}/branches/default_branch/documents/{doc_id}"
                existing = db.query(Document).filter(Document.document_name == doc_name).first()
                if not existing:
                    new_doc = Document(
                    ...
                    logger.info(f"Saved document to DB (Vertex AI Search): {file.filename} -> {doc_name} (import: {_import_elapsed:.1f}s, total: {_total_elapsed:.1f}s)")
            ...
            _total_elapsed = _time.time() - _upload_start
            logger.info(f"Upload response sent (Vertex AI Search): {file.filename} ({_total_elapsed:.1f}s)")
                ...
                if doc_name:
                    ...
                    if not existing:
                        ...
                        if corpus_record:
                            ...
                            db.add(new_doc)
                            db.commit()
                            logger.info(f"Saved document to DB: {file.filename} -> {doc_name}")
        ...
        raise HTTPException(status_code=500, detail=f"Error uploading to Gemini: {str(e)}")
    finally:
        ...
        if status_data.get("done") and status_data.get("document"):
            ...
            if doc_name:
                ...
                if not existing:
                    ...
                    final_display_name = doc_info.get("displayName") or display_name or doc_name.split("/")[-1]
                    ...
                    db.add(new_doc)
                    db.commit()
                    logger.info(f"Saved document to DB: {final_display_name} -> {doc_name}")

        return status_data
    except Exception as e:
        logger.error(f"Error checking operation status: {e}")
        raise HTTPException(status_code=500, detail=f"Error checking operation: {str(e)}")


@router.delete("/{corpus_name:path}/documents/{document_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_document(
    ...
    try:
        ...
        if search_backend == "vertex_ai_search":
            full_document_name = f"{corpus_name}/branches/default_branch/documents/{document_id}"
        else:
        ...
        try:
            ...
            logger.warning(f"External resource deletion failed for {full_document_name} (proceeding with DB cleanup): {ext_err}")

        # Delete from local DB (documents table) — 반드시 실행
        db_doc = db.query(Document).filter(
            Document.document_name == full_document_name
        ).first()
        if db_doc:
            ...
            if db_doc.gcs_path:
                try:
                    from ..services import gcs_service
                    if gcs_service.is_configured(tenant_id=current_user.tenant_id, db=db):
                        gcs_service.delete_file(db_doc.gcs_path, tenant_id=current_user.tenant_id, db=db)
                except Exception as gcs_err:
                    logger.warning(f"GCS delete failed for {full_document_name}: {gcs_err}")
            db.delete(db_doc)
...
async def download_document(
    ...
    return {"view_url": view_url, "download_url": download_url, "display_name": doc.display_name}
...
async def get_document_names(
    ...
    try:
        ...
        # display_name만 조회 (빠름)
        names = db.query(Document.display_name).filter(
            Document.corpus_id == corpus.id
        ).all()

        name_list = [n[0] for n in names if n[0]]

        logger.info(f"Retrieved {len(name_list)} document names from DB for {corpus_name}")
...
@router.post("/{corpus_name:path}/documents/bulk-delete", response_model=BulkDeleteResponse)
async def bulk_delete_documents(
    ...
    try:
        ...
        if search_backend == "vertex_ai_search":
            ...
            corpus_record = db.query(CorpusModel).filter(CorpusModel.corpus_name == corpus_name).first()
            for display_name in request.display_names:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"Vertex AI Search delete failed for {display_name} (continuing): {search_err}")
                    ...
                    if doc.gcs_path:
                        try:
                            from ..services import gcs_service
                            if gcs_service.is_configured(tenant_id=current_user.tenant_id, db=db):
                                gcs_service.delete_file(doc.gcs_path, tenant_id=current_user.tenant_id, db=db)
                        except Exception as gcs_err:

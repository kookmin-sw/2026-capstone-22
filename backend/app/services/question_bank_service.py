...
④ 어휘: 단어 뜻·숙어·동의어·반의어·어휘 자체를 직접 묻거나, 선택지가 모두 단어/표현 단위인 문제.
   → 지문을 전체적으로 읽지 않아도 빈칸 주변 몇 문장만으로 답이 결정되는 경우.
⑤ 독해: 지문 또는 대화 전체의 흐름·논리·맥락을 파악해야 풀 수 있는 문제.
   → 단어 뜻만으로는 부족하고, 글 전체를 읽어야 적절한 표현/순서/삽입 위치를 알 수 있는 경우.

빈칸 문제 area 3-way 판단 (가장 자주 헷갈리는 유형):
- 문법 빈칸: "빈칸에 알맞은 형태" / 어형 변화·to부정사·분사 등 → area=문법
- 어휘 빈칸: 선택지가 단어/숙어이고, 단어 의미 자체가 답 결정 → area=어휘
- 빈칸 추론: 선택지가 절·구이거나, 지문 논리·흐름을 따라야 답이 도출 → area=독해

대화문 빈칸 판단:
- 대화 흐름·상황·의도를 파악해야 → area=독해 (빈칸 추론 or 내용 일치)
- 대화 속 단어 의미를 직접 묻는다면 → area=어휘

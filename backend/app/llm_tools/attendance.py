...
ATTENDANCE_FUNCTION_DECLARATIONS = [
    {
        ...
        "description": "연결된 학생의 기간 내 출결 현황 요약(출석/결석/지각/조퇴 횟수, 출석률)을 조회합니다.",
        "parameters": {
            "type": "object",
            "properties": {
                "from_date": {
    ...
    },
    {
        ...
        "description": "연결된 학생의 기간 내 출결 상세 기록(날짜, 상태, 메모)을 조회합니다.",
        "parameters": {
            "type": "object",
            "properties": {
                "from_date": {
...
def execute_attendance_tool(
    ...
    """출결 tool executor. 에러 dict 변환은 모두 이 함수에서 처리한다.
    ...
    # 3. 함수 분기
    if function_name == "get_my_attendance_summary":
        return fetch_attendance_summary(
            db=db,
            tenant_id=tenant_id,
    ...
    if function_name == "get_my_attendance_records":
        # status_filter enum 검증
        status_filter_str: Optional[str] = args.get("status_filter")
        status_filter = None
        if status_filter_str is not None:
            try:
                status_filter = AttendanceStatus(status_filter_str)
            except ValueError:
                return {
                    "error": "invalid_status_filter",
                    "message": f"'{status_filter_str}'은 유효하지 않은 출결 상태입니다.",
    ...
    logger.warning("Unknown attendance function: %s", function_name)

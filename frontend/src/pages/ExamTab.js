...
const DUMMY_EXAMS = [
  ...
  { id: 5, title: '물리: 역학의 법칙 테스트', subject: '과학', date: '2026-04-03', class_id: 3, max_score: 100, type: '단원', memo: '' },
  { id: 6, title: '전국 연합 모의고사(3월)', subject: '수학', date: '2026-03-25', class_id: 1, max_score: 100, type: '외부', memo: '비교 데이터용' },
];

const DUMMY_RESULTS = [
  // Exam 1 Results
  { id: 1, exam_id: 1, student_id: 1, score: 95, grade: '1', rank: 1, comment: '우수한 성적입니다.', updated_at: '2026-04-02' },
  { id: 2, exam_id: 1, student_id: 2, score: 88, grade: '2', rank: 2, comment: '실수 보완 필요.', updated_at: '2026-04-02' },
  { id: 3, exam_id: 1, student_id: 3, score: 72, grade: '3', rank: 4, comment: '기본기 복습 요망.', updated_at: '2026-04-02' },
  { id: 4, exam_id: 1, student_id: 4, score: 84, grade: '2', rank: 3, comment: '지난 시험 대비 상승.', updated_at: '2026-04-02' },
  // Exam 6 Results (for comparison)
  { id: 100, exam_id: 6, student_id: 1, score: 92, grade: '1', rank: 1, comment: '', updated_at: '2026-03-26' },
  { id: 101, exam_id: 6, student_id: 2, score: 90, grade: '1', rank: 2, comment: '', updated_at: '2026-03-26' },
];
...
export default function ExamTab() {
  ...
  const [filters, setFilters] = useState({
    class_id: 'all', student_name: '', subject: 'all', dateStart: '', dateEnd: '',
  });
  ...
  const stats = useMemo(() => {
    ...
    if (res.length === 0) return { avg: 0, max: 0, min: 0, count: 0, decliners: 0, distribution: [] };
    ...
    // 성적 하락 학생 수 (이전 시험 대비) - 목데이터 기준 시뮬레이션
    const decliners = res.filter(r => r.id === 2 || r.id === 3).length; // 임의 지정

    return { avg, max, count: res.length, decliners, distribution };
  }, [results, selectedExamId]);

  // 상단 요약 카드 데이터
  const summary = useMemo(() => {
    const recentCount = exams.filter(e => new Date(e.date) > new Date('2026-03-01')).length;
  ...
  const currentResults = useMemo(() => {
    ...
    return classStudents.map(student => {
      ...
      return { ...student, result: res || { score: '', grade: '', rank: '', comment: '', updated_at: '' } };
    }).filter(item => !filters.student_name || item.name.includes(filters.student_name));
  }, [selectedExamId, selectedExam, results, filters.student_name]);

  // 핸들러
  const handleOpenAddExam = () => { setEditingExam(null); setExamForm({ title: '', subject: '', date: '', class_id: '', max_score: 100, type: '정기', memo: '' }); setExamDialogOpen(true); };
  const handleOpenEditExam = (e, exam) => { e.stopPropagation(); setEditingExam(exam); setExamForm({ ...exam }); setExamDialogOpen(true); };
  ...
  const handleDeleteClick = (e, exam) => { e.stopPropagation(); setExamToDelete(exam); setDeleteConfirmOpen(true); };
  ...
  return (
    <Box sx={{ animation: 'fadeUp 0.4s cubic-bezier(0.16,1,0.3,1) both' }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          { label: '최근 시험 수', value: summary.recentCount, color: '#a78bfa', icon: <ExamIcon /> },
          { label: '선택 시험 평균', value: summary.avg, color: '#4ade80', icon: <TrendingUpIcon /> },
          { label: '최고 점수', value: summary.max, color: '#f59e0b', icon: <TrophyIcon /> },
      ...
      <Box sx={{ bgcolor: '#18181B', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', p: 2.5, mb: 3, display: 'flex', gap: 1.5, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <TextField type="date" size="small" label="시험일 시작" value={filters.dateStart} onChange={e => setFilters(p => ({ ...p, dateStart: e.target.value }))} sx={{ ...inputSx, minWidth: 150 }} InputLabelProps={{ shrink: true }} />
        <TextField type="date" size="small" label="시험일 종료" value={filters.dateEnd} onChange={e => setFilters(p => ({ ...p, dateEnd: e.target.value }))} sx={{ ...inputSx, minWidth: 150 }} InputLabelProps={{ shrink: true }} />
        ...
        <Box sx={{ flex: 1 }} />
        <Button variant="contained" startIcon={<AddIcon />} onClick={handleOpenAddExam} sx={{ background: 'linear-gradient(135deg, #a78bfa 0%, #7c3aed 100%)', fontWeight: 700, borderRadius: '10px', px: 2.5, py: 1, textTransform: 'none', '&:hover': { opacity: 0.9 } }}>시험 추가</Button>
      </Box>
            ...
            <Paper sx={{ bgcolor: '#18181B', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', overflow: 'hidden' }}>
              <Box sx={{ px: 3, py: 2, bgcolor: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography sx={{ color: '#FAFAFA', fontWeight: 800, fontSize: '0.9375rem' }}>시험 목록</Typography>
                </Box>
              ...
              <Box sx={{ maxHeight: 300, overflowY: 'auto' }}>
                <Table stickyHeader size="small">
                  ...
                  <TableBody>
                    {filteredExams.map(e => (
                      <TableRow key={e.id} hover onClick={() => setSelectedExamId(e.id)} sx={{ cursor: 'pointer', bgcolor: selectedExamId === e.id ? 'rgba(167,139,250,0.06)' : 'transparent', '& td': { borderBottom: '1px solid rgba(255,255,255,0.04)', color: '#A1A1AA' } }}>
                        <TableCell sx={{ fontWeight: 700, color: '#FAFAFA !important' }}>{e.title}</TableCell>
                        <TableCell><Box sx={{ display: 'flex', gap: 0.5 }}><Chip label={e.subject} size="small" sx={{ height: 20, fontSize: '0.65rem', bgcolor: 'rgba(255,255,255,0.05)', color: '#71717A' }} /><Chip label={e.type} size="small" sx={{ height: 20, fontSize: '0.65rem', bgcolor: 'rgba(167,139,250,0.1)', color: '#a78bfa' }} /></Box></TableCell>
                        ...
                        <TableCell align="right" onClick={ev => ev.stopPropagation()}>
                          <IconButton size="small" onClick={(ev) => handleOpenEditExam(ev, e)} sx={{ color: '#52525B', '&:hover': { color: '#a78bfa' } }}><EditIcon fontSize="small" /></IconButton>
                          <IconButton size="small" onClick={(ev) => handleDeleteClick(ev, e)} sx={{ color: '#52525B', '&:hover': { color: '#ef4444' } }}><DeleteIcon fontSize="small" /></IconButton>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
            ...
            <Paper sx={{ bgcolor: '#18181B', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', overflow: 'hidden' }}>
              <Box sx={{ px: 3, py: 2, bgcolor: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography sx={{ color: '#FAFAFA', fontWeight: 800, fontSize: '0.9375rem' }}>학생별 시험 결과 {selectedExam ? `[${selectedExam.title}]` : ''}</Typography>
                </Box>
                  ...
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell sx={{ bgcolor: '#111113', color: '#71717A', fontWeight: 800, fontSize: '0.75rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>반 순위</TableCell>
                    ...
                    <TableBody>
                      {currentResults.map(item => (
                        <TableRow key={item.id} sx={{ '& td': { borderBottom: '1px solid rgba(255,255,255,0.04)', color: '#A1A1AA', py: 1.5 } }}>
                          ...
                          <TableCell><TextField size="small" type="number" value={item.result.score} onChange={ev => updateResult(item.id, { score: Number(ev.target.value) })} sx={{ width: 80, '& .MuiOutlinedInput-root': { bgcolor: 'transparent', fontSize: '0.8125rem' }, '& .MuiOutlinedInput-input': { p: 0.5, textAlign: 'center', fontWeight: 700, color: '#a78bfa' } }} /></TableCell>
                          ...
                          <TableCell><TextField size="small" type="number" value={item.result.rank} onChange={ev => updateResult(item.id, { rank: Number(ev.target.value) })} sx={{ width: 60, '& .MuiOutlinedInput-root': { bgcolor: 'transparent', fontSize: '0.8125rem' } }} /></TableCell>
                          <TableCell><TextField fullWidth size="small" value={item.result.comment} onChange={ev => updateResult(item.id, { comment: ev.target.value })} placeholder="피드백 입력..." sx={{ '& .MuiOutlinedInput-root': { bgcolor: 'transparent', fontSize: '0.8125rem' } }} /></TableCell>
        ...
        <Grid item xs={12} lg={3}>
          <Box sx={{ position: 'sticky', top: 20, display: 'flex', flexDirection: 'column', gap: 3 }}>
            <Paper sx={{ bgcolor: '#18181B', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}><BarChartIcon sx={{ color: '#a78bfa', fontSize: 20 }} /><Typography sx={{ color: '#FAFAFA', fontWeight: 800, fontSize: '0.875rem' }}>점수 분포 현황</Typography></Box>
              {selectedExamId ? (
                <>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}><Typography sx={{ color: '#71717A', fontSize: '0.8125rem' }}>응시 인원</Typography><Typography sx={{ color: '#FAFAFA', fontWeight: 800 }}>{stats.count}명</Typography></Box>
                  ...
                  <Box sx={{ mt: 3, p: 2, borderRadius: '12px', bgcolor: 'rgba(239,68,68,0.05)', border: '1px dashed rgba(239,68,68,0.2)', display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <TrendingDownIcon sx={{ color: '#ef4444', fontSize: 18 }} /><Typography sx={{ color: '#fca5a5', fontSize: '0.75rem', fontWeight: 600, lineHeight: 1.4 }}>전회 대비 성적 하락 학생 <Typography component="span" sx={{ fontWeight: 900, textDecoration: 'underline' }}>{stats.decliners}명</Typography> 발견. 집중 상담 권장.</Typography>
                  </Box>
                </>
              ) : (
                <Box sx={{ py: 10, textAlign: 'center' }}><InfoIcon sx={{ fontSize: 32, color: '#27272A', mb: 1.5 }} /><Typography sx={{ color: '#52525B', fontSize: '0.75rem' }}>시험을 선택하면<br/>상세 분석이 표시됩니다</Typography></Box>
              )}
            </Paper>
            <Paper sx={{ bgcolor: '#18181B', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}><GroupsIcon sx={{ color: '#a78bfa', fontSize: 20 }} /><Typography sx={{ color: '#FAFAFA', fontWeight: 800, fontSize: '0.875rem' }}>분반별 평균 비교</Typography></Box>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {DUMMY_CLASSES.map(c => (
                  <Box key={c.id} sx={{ p: 1.5, borderRadius: '10px', bgcolor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}><Typography sx={{ fontSize: '0.75rem', color: '#A1A1AA' }}>{c.name}</Typography><Typography sx={{ fontSize: '0.75rem', color: '#FAFAFA', fontWeight: 700 }}>{80 + c.id * 2}점</Typography></Box>
                    <Box sx={{ height: 4, width: '100%', bgcolor: 'rgba(255,255,255,0.05)', borderRadius: 2 }}><Box sx={{ width: `${80 + c.id * 2}%`, height: '100%', bgcolor: c.id === 1 ? '#a78bfa' : '#3f3f46', borderRadius: 2 }} /></Box>
                  </Box>
                ))}
              </Box>
            </Paper>
      ...
      <Dialog open={examDialogOpen} onClose={() => setExamDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: '#18181B', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px' } }}>
        ...
        <DialogContent sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
            ...
            <FormControl size="small"><InputLabel sx={{ color: '#71717A' }}>시험 유형</InputLabel><Select value={examForm.type} onChange={e => setExamForm(p => ({ ...p, type: e.target.value }))} label="시험 유형" sx={selectSx} MenuProps={menuProps}><MenuItem value="정기">정기</MenuItem><MenuItem value="단원">단원</MenuItem><MenuItem value="외부">외부</MenuItem><MenuItem value="이벤트">이벤트</MenuItem></Select></FormControl>
          </Box>
      ...
      <Dialog open={deleteConfirmOpen} onClose={() => setDeleteConfirmOpen(false)} PaperProps={{ sx: { bgcolor: '#18181B', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px' } }}>
        ...
        <DialogActions sx={{ p: 2.5 }}><Button onClick={() => setDeleteConfirmOpen(false)} sx={{ color: '#71717A' }}>취소</Button><Button onClick={handleConfirmDelete} sx={{ bgcolor: 'rgba(239,68,68,0.1)', color: '#ef4444', fontWeight: 700 }}>삭제</Button></DialogActions>
      </Dialog>
